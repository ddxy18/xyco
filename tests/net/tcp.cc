...
TEST(TcpTest, reuseaddr) {
  TestRuntimeCtx::co_run([]() -> xyco::runtime::Future<void> {
    ...
    {
      auto tcp_socket1 = xyco::net::epoll::TcpSocket::new_v4().unwrap();
    ...
    {
      auto tcp_socket2 = xyco::net::epoll::TcpSocket::new_v4().unwrap();
...
TEST(TcpTest, bind_same_addr) {
  TestRuntimeCtx::co_run([]() -> xyco::runtime::Future<void> {
    ...
    auto result1 = co_await xyco::net::epoll::TcpListener::bind(
        xyco::net::SocketAddr::new_v4(ip, port));

    auto result2 = co_await xyco::net::epoll::TcpListener::bind(
        xyco::net::SocketAddr::new_v4(ip, port));
...
TEST(TcpTest, TcpListener_bind) {
  TestRuntimeCtx::co_run([]() -> xyco::runtime::Future<void> {
    ...
    auto result = co_await xyco::net::epoll::TcpListener::bind(
        xyco::net::SocketAddr::new_v4(ip, port));
...
TEST(TcpTest, connect_to_closed_server) {
  TestRuntimeCtx::co_run([]() -> xyco::runtime::Future<void> {
    ...
    auto client = co_await xyco::net::epoll::TcpStream::connect(
        xyco::net::SocketAddr::new_v4(ip, port));
    ...
    TestRuntimeCtx::co_run([]() -> xyco::runtime::Future<void> {
      ...
      listener_ = std::make_unique<xyco::net::epoll::TcpListener>(
          xyco::net::epoll::TcpListener(
              (co_await tcp_socket.listen(1)).unwrap()));
    });
  }
...
std::unique_ptr<xyco::net::epoll::TcpListener> WithServerTest::listener_;
...
TEST_F(WithServerTest, TcpSocket_connect) {
  TestRuntimeCtx::co_run([]() -> xyco::runtime::Future<void> {
    auto client =
        co_await xyco::net::epoll::TcpSocket::new_v4().unwrap().connect(
...
TEST_F(WithServerTest, TcpSocket_new_v6) {
  ASSERT_EQ(xyco::net::epoll::TcpSocket::new_v6().is_ok(), true);
}
