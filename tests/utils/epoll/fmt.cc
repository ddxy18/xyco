...
TEST(FmtTypeTest, IoExtra_Event) {
  auto event = xyco::runtime::Event{
      .extra_ = std::make_unique<io::IoExtra>(io::IoExtra::Interest::All, 4)};
  ...
  extra->state_.set_field<io::IoExtra::State::Readable>();
  extra->state_.set_field<io::IoExtra::State::Writable>();
  fmt_str = fmt::format("{}", event);
  ASSERT_EQ(fmt_str,
  ...
  extra->state_.set_field<io::IoExtra::State::Writable, false>();
  extra->state_.set_field<io::IoExtra::State::Pending>();
  fmt_str = fmt::format("{}", event);
  ASSERT_EQ(fmt_str,
            "Event{extra_=IoExtra{state_=[Registered,Pending], interest_=All, "
            "fd_=4}}");

  extra->state_.set_field<io::IoExtra::State::Pending, false>();
  extra->state_.set_field<io::IoExtra::State::Readable>();
  ...
  extra->state_.set_field<io::IoExtra::State::Writable, false>();
  extra->state_.set_field<io::IoExtra::State::Error>();
  fmt_str = fmt::format("{}", event);
  ASSERT_EQ(fmt_str,
