...
auto xyco::logging::LoggerCtx::get_logger() -> std::shared_ptr<spdlog::logger> {
  auto configure = []() {
    ...
    auto file_sink = std::make_shared<spdlog::sinks::daily_file_sink_mt>("runtime.txt", 0, 0, true);

    auto logger =
        std::make_shared<spdlog::logger>(spdlog::logger("xyco", {console_sink, file_sink}));
