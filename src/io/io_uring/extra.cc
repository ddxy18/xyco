...
auto fmt::formatter<xyco::io::uring::IoExtra::Accept>::format(
  ...
  auto sock_addr = xyco::net::SocketAddr::new_v4(
      xyco::net::Ipv4Addr(
          ::inet_ntop(addr->sin_family, &addr->sin_addr, ip.data(), ip.size())),
      addr->sin_port);
  return format_to(ctx.out(), "Accept{{addr_={}, flags_={}}}", sock_addr,
                   args.flags_);
}
...
auto fmt::formatter<xyco::io::uring::IoExtra::Connect>::format(
  ...
  auto sock_addr = xyco::net::SocketAddr::new_v4(
      xyco::net::Ipv4Addr(
          inet_ntop(addr->sin_family, &addr->sin_addr, ip.data(), ip.size())),
      addr->sin_port);
  return format_to(ctx.out(), "Connect{{addr_={}}}", sock_addr);
}
