...
auto xyco::net::epoll::TcpSocket::new_v4() -> io::IoResult<TcpSocket> {
  return io::into_sys_result(::socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0))
      .map([](auto fd) { return TcpSocket(fd); });
}

auto xyco::net::epoll::TcpSocket::new_v6() -> io::IoResult<TcpSocket> {
...
auto xyco::net::epoll::TcpStream::flush() -> Future<io::IoResult<void>> {
