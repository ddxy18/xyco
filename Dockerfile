...
RUN ln -snf /usr/share/zoneinfo/$CONTAINER_TIMEZONE /etc/localtime && echo $CONTAINER_TIMEZONE > /etc/timezone \
    ...
    clang-$llvm_version clang-format-$llvm_version clang-tidy-$llvm_version clangd-$llvm_version \
    libc++-$llvm_version-dev libc++abi-$llvm_version-dev libunwind-$llvm_version-dev \
    && ln -s /usr/bin/clang-$llvm_version /usr/bin/clang && ln -s /usr/bin/clang-format-$llvm_version /usr/bin/clang-format \
